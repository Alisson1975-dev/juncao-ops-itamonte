...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'juncao-ops-v1';

/**
 * APLICAÇÃO FINAL: JUNÇÃO DE OPs
 * Versão Final com botões de "Voltar" em todas as interações.
 */
export default function App() {
  ...
  // Nomes dos botões de transformação
  ...
  const handleJuntarQuantidades = useCallback(() => {
    ...
    const newData = data.map((r, i) => {
      if (i >= 300) {
        const item = ag[i - 300];
        return item ? { ...r, sequencia: item.s, item: item.i, quantidade: item.q.toLocaleString('pt-PT', { minimumFractionDigits: 2 }), data: item.d, ordemProducao: "" } : { ...r, sequencia: "", item: "", quantidade: "", data: "", ordemProducao: "" };
      }
      return r;
    });
    setData(newData);
  ...
  const processPastedText = useCallback((text, target) => {
    ...
    if (target === 'geral') {
      ...
      showActionMessage(`${lines.length} linhas importadas!`);
    } else if (target === 'op') {
      lines.forEach((line, i) => { if (i + 300 < 600) newData[i+300].ordemProducao = line.trim(); });
      showActionMessage(`${lines.length} OPs coladas!`);
    }
    setData(newData);
  ...
  const handlePasteAction = async (target) => {
    try {
      ...
      else showActionMessage("Área de transferência vazia.", "error");
    } catch (err) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      } catch (err) { showActionMessage("Erro no Excel.", "error"); }
    };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(configDoc, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (cloudLabels && Array.isArray(cloudLabels)) setBtnLabels(cloudLabels);
      }
      ...
      {showManualPaste && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 text-center">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between mb-4 font-black uppercase">
              <h3>Colagem Manual</h3>
              <button onClick={() => setShowManualPaste(false)} className="text-slate-400 hover:text-red-500 font-bold text-xl">X</button>
            </div>
            <textarea 
              ref={manualPasteRef} 
            ...
            <div className="space-y-4 text-left">
              {btnLabels.map((label, idx) => (
                <div key={idx} className="flex flex-col gap-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase">Prefixo do Botão {idx + 1}</label>
                  <input 
                    type="text" 
                    value={label} 
                    maxLength={5} 
      ...
      <div className="max-w-[1800px] mx-auto w-full flex flex-col h-[calc(100vh-2rem)] gap-4">
                ...
                <div className="relative">
                  {!showClearConfirm ? (
                    ...
                    <div className="flex items-center gap-1 bg-red-600 text-white rounded-lg p-0.5 shadow-lg">
                      <span className="text-[9px] font-black px-1 uppercase ml-2">Certeza?</span>
        ...
        <div className="flex-grow flex flex-col md:flex-row gap-4 overflow-hidden px-1 relative">
          ...
          <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full min-w-[450px]">
            <div className="bg-blue-600 px-4 py-2 flex items-center justify-between shadow-md z-20 relative text-white text-center">
          ...
          {/* Parte 02 */}
          <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm h-full min-w-[450px]">
            <div className="bg-green-600 px-4 py-2 flex items-center justify-between shadow-md z-20 relative text-white text-center">
        ...
        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4 text-indigo-700 text-[10px] font-black uppercase">
            <span className="bg-indigo-50 px-3 py-1 rounded-lg border border-indigo-100 shadow-inner">Registos: {filledCount} / 600</span>
          </div>
