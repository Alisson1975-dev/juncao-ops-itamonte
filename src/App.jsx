    ...
    reader.onload = (evt) => {
      try {
        ...
        rows.slice(0, 300).forEach((r, i) => { 
          if (Array.isArray(r)) {
            newData[i] = { ...newData[i], item: r[0] ? String(r[0]).trim() : "", quantidade: r[1] ? String(r[1]).trim() : "", data: r[2] ? String(r[2]).trim() : "" }; 
          }
        });
        setData(newData);
      ...
      {showManualPaste && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full">
            ...
            <button onClick={() => setShowManualPaste(false)} className="mt-4 w-full py-2 bg-slate-100 rounded-xl font-black uppercase text-xs">Voltar</button>
          </div>
        </div>
      )}
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm shrink-0">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 flex-grow">
              ...
              {!showClearConfirm ? (
                <button onClick={() => setShowClearConfirm(true)} className="px-4 py-1.5 bg-red-50 text-red-600 border border-red-100 rounded-lg text-[10px] font-black uppercase text-center">Limpar</button>
              ) : (
                <div className="flex items-center gap-1 bg-red-600 text-white rounded-lg p-0.5">
                  ...
                  <button onClick={() => { setData(Array.from({ length: 600 }, (_, i) => ({ id: i + 1, sequencia: "", tabela: i < 300 ? 'azul' : 'verde', item: "", quantidade: "", data: "", ordemProducao: "" }))); setShowClearConfirm(false); }} className="px-2 py-1 font-bold text-[10px]">Sim</button>
